...
const DEPARTMENT_ADMIN_TILES = {
  ...
  purchasing: [
    ...
    {view:'newOnShelfUpload', icon:'🆕', title:'העלאת חדש על המדף', sub:'אוטומטי מ-Word'},
...
function adminPromoItemsUpload(){
  const count = Object.keys(appData.purchasingPromoItems||{}).length;
  return `
    <div class="card">
      <p style="font-size:12.5px;color:var(--text-secondary);margin:0 0 16px;">
        קובץ ברקודים ושמות מוצרים לכל קוד מבצע (JSON מוכן, מופק מפענוח חוברת
        המבצעים) — משמש את מסך הצ'ק-ליסט של הסניפים כדי להציג ברקוד ושם מוצר
        לכל פריט. קובץ חדש מחליף את הישן במלואו.
      </p>
      <div style="font-size:13px;color:var(--text-secondary);margin-bottom:14px;">
        כרגע במערכת: <b>${count}</b> ברקודים.
      </div>
      <input type="file" id="promo-items-input" accept=".json" style="display:none;" onchange="handlePromoItemsUpload(event)">
      <button class="btn-import" onclick="document.getElementById('promo-items-input').click()">📥 העלאת קובץ פריטי מבצע (JSON)</button>
      <button class="icon-btn" style="width:auto;padding:8px 14px;font-size:12.5px;margin-inline-start:8px;color:var(--critical,#d03b3b);" onclick="resetPromoItems()">🗑️ איפוס פריטי מבצע (מחיקת הכל)</button>
      <div id="promo-items-status" style="margin-top:12px;font-size:13px;"></div>
    </div>
  `;
}

function resetPromoItems(){
  if(!confirm('למחוק את כל פריטי המבצע הקיימים? הפעולה בלתי הפיכה - תצטרכו להעלות קובץ JSON מחדש.')) return;
  const statusEl = document.getElementById('promo-items-status');
  if(statusEl) statusEl.textContent = 'מוחק פריטי מבצע...';
  db.collection('purchasingPromoItems').get().then(function(snap){
    const delBatches = [];
    let batch = db.batch(); let n=0;
    snap.forEach(function(doc){
      batch.delete(doc.ref); n++;
    ...
    if(statusEl) statusEl.textContent = 'כל פריטי המבצע נמחקו.';
  }).catch(function(err){
    if(statusEl) statusEl.textContent = 'שגיאה במחיקה: ' + err.message;
  });
}

/* ייבוא פריטי מבצע - קובץ JSON קטן {items:{ברקוד: {code,name,dept,group}}}
   שמופק מפענוח חוברת המבצעים (ראו השיחה על פענוח PDF). מחליף לגמרי את
   הקולקציה purchasingPromoItems (לא promoProducts!). */
function handlePromoItemsUpload(evt){
  const file = evt.target.files[0];
  if(!file) return;
  const statusEl = document.getElementById('promo-items-status');
  if(statusEl) statusEl.textContent = 'קורא את קובץ ה-JSON...';
  ...
  reader.onload = function(e){
    ...
    try{
      parsed = JSON.parse(e.target.result);
      if(!parsed || typeof parsed!=='object' || !parsed.items) throw new Error('מבנה קובץ לא תקין - חסר שדה items');
    }catch(err){
      if(statusEl) statusEl.textContent = 'שגיאה בקריאת הקובץ: ' + err.message;
      return;
    }
    const items = parsed.items;
    if(statusEl) statusEl.textContent = `נמחקים ${Object.keys(appData.purchasingPromoItems||{}).length} רשומות ישנות...`;
    db.collection('purchasingPromoItems').get().then(function(snap){
      const delBatches = [];
      let batch = db.batch(); let n=0;
      snap.forEach(function(doc){
      ...
      if(statusEl) statusEl.textContent = `כותב ${barcodes.length} פריטים חדשים...`;
      const writeBatches = [];
      let batch = db.batch(); let n=0;
      ...
      if(statusEl) statusEl.textContent = `הועלו ${Object.keys(items).length} פריטים בהצלחה.`;
    }).catch(function(err){
      if(statusEl) statusEl.textContent = 'שגיאה בכתיבה: ' + err.message;
    });
  };
  reader.readAsText(file);
}
